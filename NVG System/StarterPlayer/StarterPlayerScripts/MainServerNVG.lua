...
local success, result = pcall(function()
	return require(script:WaitForChild("NVGVerticalLift"))
end)
...
-- Centralize vertical lift state control
local function updateVerticalLiftState(isOn)
	if verticalLiftSystem then
		verticalLiftSystem:SetEnabled(not isOn)
	end
end
...
function initializeVerticalLift()
	if helmet and gui and VerticalLift then
		...
		if success then
			...
			updateVerticalLiftState(nvgactive)
		else
...
function togglenvg(bool)
	...
	if not animating and nvg then
		...
		-- Degree-based last-angle flow
		if verticalLiftSystem then
			if bool == true then
				lastUserAngle = verticalLiftSystem:GetCurrentAngle()
				local minAngle = verticalLiftSystem:GetAngleRange()
				-- GetAngleRange returns (min, max); when captured in one variable it yields min
				verticalLiftSystem:ForceTweenToAngle(minAngle, 0.3)
			else
				verticalLiftSystem:ForceTweenToAngle(lastUserAngle, 0.3)
			end
		end
